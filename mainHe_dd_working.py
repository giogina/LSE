...
def A_matrix(a, a_factor, n_max, n_factor=None):
    ...
    # --- DD implementation (mirrors D_matrix style) ---
    a = np.asarray(a, dtype=np.float64)
    a_factor = np.asarray(a_factor, dtype=np.float64)
    if a_factor.shape != a.shape:
        raise ValueError(f"A_matrix: a_factor must have shape {a.shape}, got {a_factor.shape}")
...
def kron_3d(wa, amat, dmat):
    ...
    wa_dd = dd_from(wa)
    Wa = (wa_dd[0][None, None, None, None, :],
          wa_dd[1][None, None, None, None, :])
...
amat_S = A_matrix(a[0], np.ones_like(a[0]), n_max) # todo: let A_matrix accept dd a
...
def build_term(
        const = 1.0,
        a_dep = np.ones_like(a[0]), # todo: temp
        delta_h = -2,
        delta_l = 0,
        delta_k = 0,
...
terms.append(build_term(
    delta_h=-1,
    delta_k=-2,
    a_dep=dd_mul_exact_scalar(dd_mul(a, vt), alpha)[0],
    factor_k= lambda k: k
))
...
terms.append(build_term(
    const = dd_subs(dd_from(0.5), Minv)[0],
    factor_k=lambda k: k**2
))

# (M1M+Minv-1)*h*k/r^2
terms.append(build_term(
    const = dd_mul_exact_scalar(Minv, 2.0)[0],
    factor_k=lambda k:  k,
...
terms.append(build_term(
    delta_h=-1,
    a_dep=dd_mul_exact_scalar(dd_add(dd_mul(dd_mul_exact_scalar(M1M, 0.5), qs), dd_mul(dd_subs(Minv, one),vs)), -alpha)[0],
    factor_k=lambda k: k
))
...
# (2*M1M-Minv)*k/r^2
terms.append(build_term(
    a_dep = np.ones_like(a) * (M1M + 1.0),
    factor_k=lambda k:  k
))

# (-4*Minv+4)*n*k/r^2
terms.append(build_term(
    a_dep = np.ones_like(a) * (-4*Minv + 4.0),
    factor_n=lambda n:  n,
    factor_k=lambda k: k
))
...
terms.append(build_term(
    a_dep = -(0.5)*(M1M+Minv) * np.ones_like(a),
    factor_hl=lambda h, l: h**2
))

# (-M1M-Minv)*h*l/(ln(r)*r^2)
terms.append(build_term(
    a_dep = -(1.0 + 2.0*Minv) * np.ones_like(a),
    delta_l = -1,
...
# (-2*M1M+Minv)*h/r^2
terms.append(build_term(
    a_dep = (-2*M1M + Minv) * np.ones_like(a),
    factor_hl=lambda h, l:  h
))

# 4*Minv*h*n/r^2
terms.append(build_term(
    a_dep = 4*Minv * np.ones_like(a),
    factor_n=lambda n: n,
    factor_hl=lambda h, l: h
))
...
terms.append(build_term(
    a_dep = (-(0.5)*M1M-(0.5)*Minv) * np.ones_like(a),
    delta_l=-2,
...
for term in terms:
    print(term["a_dep"])
    term["admat"] = kron_3d(term["const"]*wa,
