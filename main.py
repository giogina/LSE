                ...
                for m in frange(0, n_max, 1):  # careful: Whenever using negative indices, adjust power_table call accordingly.
                    for i in frange(0, n_max, 1):
                        for j in frange(0, n_max, 1):
                            for a in frange(0, ab_max, 1):
                                for b in frange(0, ab_max, 1):
...
matSize = h_idx.size
bSize = X[0].size if coords == "rij" else matSize
print(f"MatSize = {bSize}")
    ...
    for ks, s in enumerate(s_shells):
        ...
        for j, (s1, s2) in enumerate(zip(s1_vals, s2_vals)):
            ...
            if coords == "stmu":
                ...
                inv_rA_p  = power_table(inv_rA, ab_max)  # already full size P
                inv_rB_p  = power_table(inv_rB, ab_max)
                ...
                # Assemble basis functions from power matrices
                B = np.ones((r12_p.shape[0], matSize), dtype=np.float64) * (rAB ** h_idx)[None, :] # rA1^n*rB1^m*rA2^i*rB2^j*r12^k*rAB^h/rA^a/rB^b
                B *= rA1_p[:, n_idx]
                B *= rA2_p[:, i_idx]
                B *= rB1_p[:, m_idx]
                B *= rB2_p[:, j_idx]
            ...
            Sl += B.T @ B
            Hl_1 += B.T @ A_1
            Hl_alpha += B.T @ A_alpha
            Hl_beta += B.T @ A_beta
            Hl_alpha2 += B.T @ A_alpha2
            Hl_alphabeta += B.T @ A_alphabeta
            Hl_beta2 += B.T @ A_beta2
